...
s = 5;
% s = 0.8;
x_safe = safety_contour(2*Ntheta, R, -0.5);
...
title('Safety Margin (s = 5)', 'FontSize', 16)
xlabel('x_1')
ylabel('x_2')
xlim([-2,2])
ylim([-2,2])
...
hold on
patch(x0(1, :), x0(2, :), 'r', 'Linewidth', 3, 'EdgeColor', 'none')
title('Safety Margin (s = 1)', 'FontSize', 16)
xlabel('x_1')
ylabel('x_2')
xlim([-2,2])
ylim([-2,2])
...
for i = 1:length(clist)
    subplot(1 ,2, 1);
%     subplot(1 ,2, 2);
    x = safety_contour(Ntheta, R, -clist(i), s);
    x = Rot*x;
%     plot(x(1, :), x(2, :), 'r')
    plot(x(1, :), x(2, :), 'color', aut(i+1, :))
    
    subplot(1 ,2, 2);
    x = safety_contour(Ntheta, R, -clist(i), 1);
    x = Rot*x;
%     plot(x(1, :), x(2, :), 'r')
    plot(x(1, :), x(2, :), 'color', aut(i+1, :))
end
...
colormap(autumn(140));
% colorbar('Direction', 'reverse', 'Ticks', clist_rev, 'TickLabels', 0:-0.5:-2);
cb = colorbar('Ticks', 0:0.25:1, 'TickLabels', 0:-0.5:-2);
...
function x_safe = safety_contour(Ntheta, R, c, s)
