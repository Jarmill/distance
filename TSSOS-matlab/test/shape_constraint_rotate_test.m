...
%implement the quadratic later
S_Omega = struct('ineq', [BOX^2-omega(1:2).^2; shape_side^2 - s.^2; 1-sum(omega(3:4).^2)], 'eq', []);


%coordinate transformation
R_shape = [omega(3) -omega(4); omega(4) omega(3)];
shape_transform = omega(1:2) + R_shape*s;

%% identify monomial powers in support
w_A = replace(w, x, shape_transform);
p_shape = w_A - z;
data = get_support_monom(p_shape, S_Omega, vars, 2*d);
