...
%wasserstein (correlative sparsity)
mpol('tw1', 1, 1) 
mpol('xw1', 2, 1)
mpol('yw1', 1, 1)
eta1 = meas([tw1; xw1; yw1]);

mpol('tw2', 1, 1) 
mpol('xw2', 1, 1)
mpol('yw2', 2, 1)
eta2 = meas([tw2; xw2; yw2]);

%% support constraints
% X0_supp = x0 == X0;
% Y0_supp = y0 == Y0;
X0_supp = sum((x0-X0).^2) <= R0^2;
Y0_supp = sum((y0-Y0).^2) <= R0^2;
...
mom_peak_x = mom(mmon([tw1; xw1], d));

mom_init_y = mom(mmon([ty0; y0], d));
mom_peak_y = mom(mmon([tw2; yw2], d));
...
%CSP overlap: (t, x2, y1)
% if MOM_SUBS
%     CSP_con = mom(mmon([tw1; xw1(1); yw1], d)) == mom(mmon([tw2; xw2; yw2(1)], d));
% else
    CSP_con = mom(mmon([tw1; xw1(2); yw1], d)) - mom(mmon([tw2; xw2; yw2(1)], d))==0;
% end

mom_con = [Liou_con; CSP_con; mass(mu0x)==1];


%objective
% mom_dist = mom(sum((xw-yw).^2));
mom_dist = mom((xw1(1)-yw1(1))^2) + mom((xw2(1)-yw2(2))^2);
...
Meta1 = double(mmat(eta1));
Meta2 = double(mmat(eta2));
...
Meta1_1 = Meta1(1:(2*n+1), 1:(2*n+1));
Meta2_1 = Meta2(1:(2*n+1), 1:(2*n+1));
rank0x = rank(M0x_1, 1e-3);
rank0y = rank(M0y_1, 1e-3);
ranketa1 = rank(Meta1_1, 1e-3);
ranketa2 = rank(Meta2_1, 1e-3);

%extract putative optima if measures have low-rank moment matrices
xp_rec = double(mom(xw1));
x0_rec = double(mom(x0));
yp_rec = double(mom(yw2));
y0_rec = double(mom(y0));
tp_rec = Tmax*double(mom(tw1));


optimal_pt = all([rank0x;  rank0y; ranketa1; ranketa2]==1);
end
